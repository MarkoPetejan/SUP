  ...
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    ...
    navigator.mediaDevices.getUserMedia(constraints).then((stream) => {
      ...
      BarcodeDetector.getSupportedFormats().then((formats) => {
        const barcodeDetector = new BarcodeDetector({ formats }); // prepoznaj vse tipe kod
        ...
        const detectCode = () => {
          ...
          barcodeDetector
            ...
            .then((codes) => {
              ...
              console.log('detected count', codes.length)
              //if (codes.length == 3) {
              ...
              for (const barcode of codes) {
                console.log(barcode);
                state.barcode = barcode.rawValue;
                state.barcodeType = barcode.format;
                text.value = ''
              }
        ...
        intervalId = setInterval(() => {
          if (showDialog.value === true) detectCode()
        }, 100);
      });
    });
  }
...
watch(
  () => showDialog.value,
  (showDialog) => {
    if (showDialog === true) startVideo();
    if (showDialog === false) stopVideo();
...
<template>
  ...
  >
    ...
    <template v-slot:hint>{{ state.barcode || "Enter or scan barcode or scan QR code using button on the right" }}</template>
